# 投影片學習網（Slide Study）v2 — 明亮手繪風

**這是明亮版（奶油紙感背景、圓角粗邊框卡片、貼紙式按鈕）**，功能與深色版完全相同，
只有 `index.html` 裡的 CSS 不一樣，兩版可以互換：把這份 `index.html` 蓋掉另一份即可換皮，
`chapters.json`、`slides/`、筆記 JSON 全部通用。

字體用 Google Fonts 的 Baloo 2 + Noto Sans TC（圓體感）；若使用者的網路擋掉 Google Fonts，
會自動退回系統字型，版面不會壞掉。
